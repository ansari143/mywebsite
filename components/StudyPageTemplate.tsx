    ...
    <div className="site-page">
      ...
      <section className="rounded-3xl border border-slate-200 bg-gradient-to-br from-blue-50 via-white to-slate-50 p-6 shadow-sm sm:p-8 lg:p-10">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <Link href="/global-careers" className="site-btn-primary px-5 py-3 text-center">
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <div className="site-section p-6">
          ...
          <div className="mt-4 space-y-4 text-sm leading-7 text-slate-600 sm:text-base">
            ...
            <p>Costs vary by city, course type, and institution. Always check the official university website before applying.</p>
          </div>
        </div>
        ...
        <div className="site-section p-6 lg:col-span-2">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {guide.topCourses.map((course) => (
              <div key={course} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-medium leading-7 text-slate-700">
                {course}
              </div>
            ))}
          </div>
        ...
        <section className="site-section">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {guide.relatedBlogs.map((blog, index) => (
              <Link key={index} href={blog.link} className="site-card p-4 transition hover:shadow-md">
                <h3 className="text-base font-semibold text-slate-900">{blog.title}</h3>
                <span className="mt-2 inline-block text-sm text-blue-700">Read article →</span>
              </Link>
            ))}
          </div>
        </section>
