...
export default function ResultPage() {
  ...
  if (!data) {
    return (
      <div className="rounded-2xl bg-[#15233d] p-8 shadow-sm">
        <p className="font-semibold">No result found.</p>
        <p className="mt-2 text-white">Please take a test first.</p>
  ...
  return (
    <div className="space-y-6">
      <section className={`rounded-3xl border bg-[#15233d] p-6 shadow-sm sm:p-8 ${g.border}`}>
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="grid min-w-[240px] gap-3 sm:grid-cols-3 lg:grid-cols-1">
            <div className="rounded-2xl border border-slate-200 bg-[#15233d] p-4">
              ...
              <p className="mt-2 text-lg font-bold text-white">{currentTest?.title}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-[#15233d] p-4">
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,0.7fr)]">
        <div className="space-y-6">
          {insight && (
            <section className="rounded-3xl border border-gray-200 bg-[#15233d] p-6 shadow-sm sm:p-8">
              <h2 className="text-2xl font-bold text-white">Why this stream may fit you</h2>
              <div className="mt-5 space-y-4 text-sm leading-7 text-white sm:text-base">
                ...
                <div className="rounded-2xl bg-[#15233d] p-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {insight.schoolFocus.map((item) => (
                      <span key={item} className="rounded-full bg-[#15233d] px-3 py-1 text-sm text-white shadow-sm">
                        {item}
                      </span>
                    ))}
                  </div>
          ...
          <section className="rounded-3xl border border-gray-200 bg-[#15233d] p-6 shadow-sm sm:p-8">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-white">
                Instant guidance dashboard
              </span>
            </div>

            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-emerald-200 bg-[#15233d] p-5">
                  ...
                  {strongestTopics.length > 0 ? (
                    strongestTopics.map(([topic, value]) => (
                      <div key={topic}>
                        ...
                        <div className="mt-1 h-2 overflow-hidden rounded-full bg-[#15233d]">
                          <div className="h-full rounded-full bg-emerald-500" style={{ width: `${value.percent}%` }} />
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className="text-sm text-white">Topic-wise scores will appear here once the quiz stores detailed breakdowns.</p>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
                ...
                <div className="mt-3 space-y-3">
                  {growthTopics.length > 0 ? (
                    ...
                    <p className="text-sm text-white">Once topic-level scoring is available, this section highlights where more practice is needed.</p>
                  )}
                </div>
              </div>
            </div>
          ...
          {wrongQuestions.length > 0 && (
            <section className="rounded-3xl border border-gray-200 bg-[#15233d] p-6 shadow-sm sm:p-8">
              <h2 className="text-2xl font-bold text-white">Review Incorrect Answers</h2>
              <div className="mt-5 space-y-6">
                {wrongQuestions.map((q, index) => (
                  <div key={q.id} className="rounded-2xl border border-red-200 bg-[#15233d] p-5">
                            ...
                            return (
                              <div
                                ...
                                className={`rounded-lg border p-3 ${
                                  ...
                                  "border-gray-200 bg-[#15233d]"
                                }`}
                              >
                                <span className="font-semibold">{opt.id}.</span> {opt.text}
                                {opt.hi && <div className="mt-1 text-sm text-white">{opt.hi}</div>}
          ...
          {insight && (
            <section className="grid gap-6 xl:grid-cols-2">
              <div className={sectionClass}>
                <h2 className="text-xl font-bold text-white">Next 90 days action plan</h2>
              ...
              <div className={sectionClass}>
                <h2 className="text-xl font-bold text-white">Backup streams to compare</h2>
                <div className="mt-4 space-y-3">
                  {backup.map((item) => (
                    <Link key={item?.slug} href={`/tests/${item?.slug}`} className="block rounded-2xl border border-slate-200 p-4 transition hover:border-blue-300 hover:bg-[#15233d]">
          ...
          {insight && (
            <section className="grid gap-6 xl:grid-cols-2">
              <div className={sectionClass}>
                <h2 className="text-xl font-bold text-white">Local pathway guidance</h2>
                <div className="mt-4 space-y-3 text-sm leading-7 text-white">
              ...
              <div className={sectionClass}>
                <h2 className="text-xl font-bold text-white">Global / study abroad pathway</h2>
                <div className="mt-4 space-y-3 text-sm leading-7 text-white">
        ...
        <aside className="space-y-6">
          <section className="rounded-3xl border border-gray-200 bg-[#15233d] p-6 shadow-sm lg:sticky lg:top-24">
            <h2 className="text-lg font-bold text-white">Save or share your result</h2>
            <div className="mt-4 space-y-3">
              <div>
                <label className="text-sm font-semibold text-white">Student name (optional)</label>
                <input
                  value={studentName}
      ...
      <div
        ref={certRef}
        className="overflow-hidden rounded-[20px] bg-[#15233d] shadow-sm"
      ...
      >
        ...
        <div className="mt-4 border-y bg-gradient-to-r from-blue-50 to-purple-50 px-6 py-6 sm:px-10">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm text-white">Nishaglobal Education</p>
              <h2 className="text-2xl font-bold text-white sm:text-3xl">Certificate of Assessment</h2>
              <p className="mt-1 text-sm text-white">Career guidance • mock test • student planning</p>
            </div>
        ...
        <div className="px-6 py-8 sm:px-10">
          <p className="text-sm text-white">Presented to</p>
          <h3 className="mt-2 text-3xl font-extrabold text-white">{studentName?.trim() ? studentName.trim() : "Student"}</h3>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border p-4">
              ...
              <p className="mt-1 text-3xl font-bold text-white">{data.percent}%</p>
            </div>
            <div className="rounded-2xl border p-4">
              <p className="text-xs uppercase tracking-wide text-gray-500">Correct answers</p>
              <p className="mt-1 text-3xl font-bold text-white">{data.correct}/{data.total}</p>
            </div>
            <div className="rounded-2xl border p-4">
              <p className="text-xs uppercase tracking-wide text-gray-500">Readiness</p>
              <p className="mt-1 text-lg font-semibold text-white">{readinessBand(data.percent)}</p>
            </div>
          </div>

          <div className="mt-6 rounded-2xl bg-[#15233d] p-5 text-sm leading-7 text-white">
            {g.message}
          </div>
          ...
          <div className="mt-8 flex flex-col gap-6 border-t pt-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p suppressHydrationWarning className="mt-1 font-medium text-white">{issuedOn}</p>
            </div>
